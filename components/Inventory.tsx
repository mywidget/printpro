...
const Inventory: React.FC<InventoryProps> = ({ 
  ...
  const openModal = (mode: 'add' | 'edit' | 'restock', item?: InventoryItem) => {
    ...
    if (item) {
      ...
      // Default ke cabang aktif saat ini
      setFormData({ 
        category: 'Paper', 
        unit: 'sheet', 
        stock: 0, 
    ...
    <div className="p-4 md:p-8 pb-24">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between mb-8 gap-6">
        <div>
          ...
          <p className="text-sm text-slate-500 font-medium italic">Manajemen stok untuk {currentBranchName}</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-3">
          {isAdmin && (
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-2xl px-4 py-2 shadow-sm">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Pilih Cabang:</span>
              <select 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          <div key={item.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl hover:border-indigo-100 transition-all group relative overflow-hidden">
            ...
            {item.stock <= item.minStock && (
              <div className="mt-4 flex items-center gap-2 text-red-600 animate-pulse">
                ...
                <span className="text-[9px] font-black uppercase tracking-tighter">STOK KRITIS (Min: {item.minStock})</span>
              </div>
            )}
          </div>
        ))}
        {items.length === 0 && (
          <div className="col-span-full py-20 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
            ...
            <p className="text-slate-400 text-xs font-black uppercase tracking-widest">Gudang ini masih kosong</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white rounded-[2.5rem] w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in duration-300">
            <form onSubmit={handleSave}>
              <div className="p-8">
                <div className="flex justify-between items-center mb-8">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1 uppercase font-bold tracking-tighter">Penempatan: {currentBranchName}</p>
                  </div>
                ...
                {modalMode === 'restock' ? (
                  ...
                  <div className="space-y-5">
                    {/* KHUSUS ADMIN: Bisa pilih cabang saat tambah */}
                    {isAdmin && modalMode === 'add' && (
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase block mb-1.5 tracking-widest">Cabang Tujuan</label>
                        <select 
                    ...
                    <div>
                      <label className="text-[10px] font-black text-slate-400 uppercase block mb-1.5 tracking-widest">Nama Bahan / Material</label>
                      <input 
                        type="text" 
                        required
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase block mb-1.5 tracking-widest">Min. Alert</label>
                        <input 
                          type="number" 
                          required
                          min="0"
              ...
              <div className="p-8 bg-slate-50 flex gap-4 border-t border-slate-100">
                ...
                <button type="submit" className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-indigo-100">KONFIRMASI</button>
              </div>
            </form>
          </div>
        </div>
